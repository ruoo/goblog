...
func (this *MainController) TagController() {
	...
	tagPostQuery := tagpost.Query().Filter("tagid", tag.Id).Filter("poststatus", 0)  //当前标签查询
	//count, _ = query.Count()
	tagPostCount,_:=tagPostQuery.Count()
	if tagPostCount > 0 {

		var tp []*models.TagPost
		var pids []int64 = make([]int64, 0)

		tagPostQuery.All(&tp)  //标签拥有的文章
		for _, v := range tp {
			pids = append(pids, v.Postid)
		}
		query := new(models.Post).Query().Filter("id__in", pids)
		count,_=query.Count()
		query.OrderBy("-posttime").Limit(pagesize, (page-1)*pagesize).All(&list)
	}
	this.Data["tag"] = tag
